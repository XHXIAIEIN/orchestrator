...
rows = conn.execute("SELECT id, role, content, media_paths, created_at FROM chat_messages WHERE chat_client = 'telegram' ORDER BY id DESC LIMIT 20").fetchall()
...
3. Images: copy to `D:/Agent/tmp/tg-media/<filename>.webp`, then use Read tool to view
4. Audio: copy to `D:/Agent/tmp/tg-media/<filename>.webm`, note path for potential transcription

Example:
```bash
file tmp/media/inbound/eaf630dd.bin
# → JPEG image data, ...
cp tmp/media/inbound/eaf630dd.bin D:/Agent/tmp/tg-media/eaf630dd.webp
