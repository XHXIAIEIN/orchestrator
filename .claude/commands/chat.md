...
rows = conn.execute('SELECT role, content, created_at FROM chat_messages ORDER BY id DESC LIMIT 20').fetchall()
memory = conn.execute('SELECT summary, updated_at FROM chat_memory LIMIT 1').fetchone()
conn.close()
```
