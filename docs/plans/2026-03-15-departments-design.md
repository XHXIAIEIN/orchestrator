...
## 部门协作协议（2026-03-17 实现）

### 工部 → 刑部验收链

```
工部任务完成(done) → _dispatch_quality_review() → 创建刑部验收任务
                                                    ├─ parent_task_id 指向原工部任务
                                                    ├─ spec.observation 包含工部输出摘要
                                                    ├─ priority: medium（不抢高优先级队列）
                                                    └─ 跳过门下省审查（验收本身就是审查）
```

**防循环机制**：如果父任务的 department 已经是 `quality`，不再派生验收任务。

**数据模型变更**：tasks 表新增 `parent_task_id INTEGER` 字段，支持任务链追溯。

### 未来协作链（待实现）

- 刑部验收失败 → 自动打回工部重做（需要"重试"状态机）
- 户部运维完成 → 吏部生成绩效快照
- 兵部发现安全问题 → 自动创建工部修复任务
